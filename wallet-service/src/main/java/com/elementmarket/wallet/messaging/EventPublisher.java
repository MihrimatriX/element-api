...
public class EventPublisher {
    ...
    public void publish(String typeName, ObjectNode payload) throws Exception {
        publish(typeName, payload, null);
    }

    public void publish(String typeName, ObjectNode payload, String messageId) throws Exception {
        String exchange = MassTransitMessage.exchange(typeName);
        byte[] body = messageId == null
                ? MassTransitMessage.publishBody(objectMapper, typeName, payload)
                : MassTransitMessage.publishBody(objectMapper, typeName, payload, messageId);
