...
export async function refreshAtlas(fetchMedia=false, only=[], structuresOnly=false) {
  ...
  if(fetchMedia) {
    ...
    if(!structuresOnly) for(const records of [elements.filter(selected),compounds.filter(selected)]) for(let i=0;i<records.length;i+=5) {
      ...
      for(const r of batch) {
        ...
        if(page) {
          ...
          // Curated selections may include Z>83 metal specimens (e.g. U biscuit); pageimage heuristic stays Z<=83.
          if(r.symbol&&sample&&(selection||r.atomic_number<=83)&&(selection||new RegExp(r.names.en.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i').test(sample))&&(selection||!/portrait|diagram|electron|spectr|tube|lamp|discharge|atomic|symbol|bohr|icon/i.test(sample))) {
            try {
...
if(process.argv[1]&&import.meta.url===pathToFileURL(process.argv[1]).href)await refreshAtlas(process.argv.includes('--fetch'),process.argv.find(a=>a.startsWith('--only='))?.slice(7).split(',')??[],process.argv.includes('--structures-only'));
