...
/** AssetsCredited is async after Completed — poll holdings. */
async function holdingsReady(key, pred, label = 'holdings') {
...
try {
  ...
  const holdings = await holdingsReady(
    key,
  ...
  const saltHoldings = await holdingsReady(
    key,
