...
./deploy/scripts/test-all.ps1 -Configuration Review -Integration -Live -Browser -Recovery -WebBase http://localhost:3080
```
...
**Yayın kararı: e-postasız beta.** `SMTP_HOST` / `SMTP_FROM` boş bırakılır; şifre kurtarma ve e-posta doğrulama kapalı kalır. `GET /api/v1/auth/capabilities` `passwordRecovery: false` döner; forgot endpoint 503 ile dürüst kalır; giriş / reset UI bunu söyler. Anahtar eklenmedi, e-posta gönderilmedi.

Sağlayıcı tercihi hâlâ Resend (sonra). Mevcut mailer SMTP. Açmak için env’ye (ör. public prod):
...
Doğrulanmış alan adı gerekir. [Resend resmi SMTP belgesi](https://resend.com/docs/send-with-smtp)
