...
public sealed class AccountSecurityController(UserManager<ApplicationUser> users, IAccountMailer mailer, IConfiguration configuration,
    IdentityAppDbContext database) : ControllerBase
{
    ...
    public IActionResult Capabilities() => Ok(new { passwordRecovery = mailer.Enabled, emailVerification = mailer.Enabled });

    [HttpPost("password/forgot")]
    public async Task<IActionResult> Forgot(EmailRequest request, CancellationToken ct)
    {
        if (!mailer.Enabled) return StatusCode(503, new { message = "Bu kurulumda e-posta ile şifre kurtarma henüz yapılandırılmadı." });
