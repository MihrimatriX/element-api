...
function schema(values) {
  ...
  const variants = [...groups].map(([type, rows]) => {
    if (type === "object") {
      ...
      return {
        ...
        properties: Object.fromEntries(
          keys.map((key) => [
            key,
            schema(rows.filter((row) => key in row).map((row) => row[key])),
          ]),
        ),
    ...
    if (type === "array")
      return { type, items: rows.flat().length ? schema(rows.flat()) : {} };
    return { type };
  });
...
for (const [kind, path] of [
  [
    "elements",
    "../../catalog-service/Element.Services.Element.Infrastructure/Data/",
  ],
  [
    "compounds",
    "../../compound-service/Element.Services.Compound.Infrastructure/Data/",
  ],
]) {
  const records = JSON.parse(
    readFileSync(
      new URL(`${path}scientific-${kind}.json`, import.meta.url),
      "utf8",
    ),
  );
  writeFileSync(
