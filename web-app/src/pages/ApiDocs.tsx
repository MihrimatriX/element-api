...
const ENDPOINTS = [
  ...
  {
    path: "/api/v1/compounds?element=au",
    auth: false,
    label: "GET mağaza SKU",
  },
  {
...
export default function ApiDocs() {
  ...
  const etagUrl = publicApiUrl("/api/v2/elements/fe?fields=symbol,names");
  ...
  return (
    <main className="page api-docs-page">
      ...
      />
      <section className="hero compact api-docs-hero">
        <div>
          <h1>Deneme tezgâhı</h1>
          <p className="lead">
            Demiri veya suyu iste, JSON’u kopyala. Hesap yok, anahtar yok.
            Aşağıda uç seç, dil seç, yanıtı gör. Ürün özeti için{" "}
            <Link to="/developers">/developers</Link>; sözler için{" "}
            <Link to="/sozluk">sözlük</Link>.
          </p>
        </div>
      </section>

      <section className="screen-grid api-docs-layout">
        <aside className="side-stack docs-playground-star">
          <Card asChild className="gap-0 py-0 shadow-none">
            <div className="panel">
              <div className="panel-header docs-playground-head">
                <div>
                  <h2 style={{ margin: 0 }}>Canlı istek</h2>
                  <p className="muted" style={{ margin: "6px 0 0", fontSize: 13 }}>
                    Fe veya H₂O ile başla; yanıt paneli sağda (mobilde altta).
                  </p>
                </div>
              </div>
              <div className="playground playground-star">
                <div className="field">
                  ...
                  >
                    <optgroup label="v2 bilimsel">
                      {ENDPOINTS.filter((e) => e.path.startsWith("/api/v2")).map(
                        (e) => (
                    ...
                    {ACCOUNTS_ENABLED && (
                      <optgroup label="v1 kredi simülasyonu">
                        {ENDPOINTS.filter(
                          (e) => !e.path.startsWith("/api/v2"),
                        ).map((e) => (
                          <option key={e.path} value={e.path}>
                            {e.label}
                            {e.auth ? " (anahtar)" : ""}
                ...
                <div className="playground-panels">
                  <div style={{ position: "relative" }}>
                    ...
                    <Button
                      ...
                      onClick={() =>
                        copyToClipboard(
                          getCodeSnippet(activeLang, tryItEndpoint),
                        )
                      }
                    >
                      Kopyala
                  ...
                  <div>
                    {httpStatus != null && (
                      <p
                        ...
                        data-ok={httpStatus >= 200 && httpStatus < 400}
                      >
                        HTTP {httpStatus}
                        {etag ? ` · ${etag}` : ""}
                      </p>
                    ...
                    >
                      <code>
                        ...
                        apiResponse &&
                        "error" in (apiResponse as object)
                          ? highlightJson(
                              `${jsonSource(apiResponse)}\n\n// Gateway kapalıysa docker compose veya atlas host’u aç; sonra yeniden dene.`,
                            )
                          : highlightJson(jsonSource(apiResponse))}
                      </code>
                    </pre>
                  </div>
                </div>
        ...
        <Card asChild className="gap-0 py-0 shadow-none">
          <div className="panel padded">
            ...
            <p>
              ...
              <code>Cache-Control: public, max-age=3600</code> taşır. Aynı
              gövdenin parmak izini <code>If-None-Match</code> ile geri
              gönderirsen 304 gelir; JSON yok. Farklı <code>fields</code> farklı
              ETag üretir. CORS: GET ve OPTIONS, her origin; <code>ETag</code>{" "}
              başlığı açıktır.
            </p>
            <pre className="code-window">
            ...
            <p>
              ...
              304), one-hour cache, missing values are <code>null</code>. Rate
              limit is per host: 300/min on the atlas host (<code>:5080</code>),
              100/10s behind the gateway (<code>:5000</code>). Machine contract:{" "}
            ...
            <ul className="check-list">
              ...
              <li>
                ...
                saniyede 100. Bu tezgâh şu an {scienceDirect ? "atlas" : "kapı"}{" "}
                profilinden okuyor.
              </li>
            </ul>
            <pre className="code-window">
              ...
              <DisclosureContent>
                ...
                >
                  ...
                  <li>
                    ...
                    <code>compoundSlug</code> gönderilir. 214 eğitim bileşiği
