...
export default function Shop() {
  ...
  useEffect(() => {
    let live = true;
    compoundService
      .all({ element: onlySelected ? selectedSymbol : undefined })
      .then((data) => {
        if (!live) return;
        setCompounds(data);
        setLoadError("");
  ...
  const addSku = (sku: CompoundSku, grams: number) => {
    ...
    setCart(
      addToCart(sku.elementSymbol, grams, stock, {
        slug: sku.slug,
        formula: sku.formula,
  ...
  const overStock = cartLines.some(
    (line) =>
      cartLines
        .filter(
          (other) =>
            other.item.symbol.toUpperCase() === line.item.symbol.toUpperCase(),
        )
        .reduce((sum, other) => sum + other.qty, 0) > line.stock,
  );
  const walletShort =
    isAuthenticated && walletElx != null && subtotal > walletElx;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      for (const line of cartLines) {
        ...
        setOrdersList((prev) => [
          res,
          ...prev.filter((order) => order.id !== res.id),
        ]);
        remaining = remaining.filter(
          (item) =>
            cartLineKey(item.symbol, item.slug) !==
            cartLineKey(line.item.symbol, line.item.slug),
        );
        writeCart(remaining);
        setCart(remaining);
        accepted++;
      ...
      const detail =
        err instanceof ApiHttpError && err.status === 402
          ? "Cüzdanda yeterli kredi yok."
          : apiError(err, "Sipariş iletilemedi.");
      setCheckoutNote(
        `${detail} Kalan ürünler sepette. Güvenle tekrar deneyebilirsiniz.`,
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const orderLabel = (order: ShopOrder) => {
    if (order.productLabel)
      return `${order.productLabel} · ${fmt(order.quantity, 0)} g`;
    if (order.compoundFormula)
      return `${order.compoundFormula} · ${fmt(order.quantity, 0)} g`;
  ...
  const skuInCart = (sku: CompoundSku) =>
    cart.find(
      (c) =>
        cartLineKey(c.symbol, c.slug) ===
        cartLineKey(sku.elementSymbol, sku.slug),
    );
      ...
      <section className="shop-header">
        <div className="shop-header-copy">
          ...
          <p className="lead shop-lead">
            Sanal KREDI demosu. Gerçek para değil. Demir gramı, su SKU’su;
            bilimsel 214’lük katalog otomatik satılmaz. Paket: 1 / 10 / 100
            gram.
          </p>
          {!isAuthenticated && (
                ...
                >
                  <div className="sku-card-head">
                    ...
                    <span className="sku-kind">
                      {KIND_TR[sku.kind] || sku.kind}
                    </span>
                  </div>
                  <p className="sku-name">{sku.nameTr || sku.name}</p>
                  {sku.properties && (
                    <Disclosure className="sku-properties">
                      <DisclosureTrigger>Kimyasal özellikler</DisclosureTrigger>
                      <DisclosureContent>
                        <dl>
                          <div>
                            <dt>Molar kütle</dt>
                            <dd>
                              {fmt(sku.properties.molecularWeight, 3)} g/mol
                            </dd>
                          </div>
                          <div>
                            <dt>IUPAC adı</dt>
                            <dd>{sku.properties.iupacName}</dd>
                          </div>
                        </dl>
                        <a
                          href={sku.properties.sourceUrl}
                          target="_blank"
                          rel="noreferrer"
                        >
                          PubChem kaynağı ↗
                  ...
                  <p className="muted shop-parent">
                    ...
                    <Button
                      ...
                      className="text-link"
                      onClick={() => {
                        setSelectedSymbol(sku.elementSymbol);
                        setOnlySelected(true);
                      }}
                    >
                      {elements.find(
                        (e) =>
                          ...
                          sku.elementSymbol.toUpperCase(),
                      )?.name || sku.elementSymbol}
                    </Button>
                  </p>
                  ...
                  <span className="stock-note">
                    {line
                      ? `Sepette ${fmt(line.qty, 0)} g`
                      : `${fmt(q.stock, 0)} g stok`}
                  </span>
                  <Button
                    variant="default"
        ...
        <aside className="side-stack">
          <Card asChild className="gap-0 py-0 shadow-none">
            <section className="panel cart-card" id="cartSummary">
              <div className="panel-header">
                <div>
                  <h3 style={{ margin: 0 }}>Sepet · gramlar</h3>
                </div>
                <Button
                  variant="outline"
                  className="mini-btn"
              ...
              <div className="panel-body">
                    ...
                    <div className="cart-row-actions">
                      <div className="quantity-stepper">
                        <Button
                          ...
                          type="button"
                          disabled={submitting}
                          aria-label={`${line.item.formula} miktarını azalt`}
                          onClick={() => handleStepCart(line.item, -1)}
                        >
                          −
                        </Button>
                        ...
                        <Button
                          ...
                          type="button"
                          disabled={submitting}
                          aria-label={`${line.item.formula} miktarını artır`}
                          onClick={() => handleStepCart(line.item, 1)}
                        >
                          +
                        </Button>
                ...
                <div className="cart-totals">
                  <div className="grand">
                    <span>Toplam</span>
                    <strong>{elx(subtotal)}</strong>
                  </div>
                  {isAuthenticated && walletElx != null && (
                    <div>
                      <span>Cüzdan</span>
                      <strong>{elx(walletElx)}</strong>
                    </div>
                  )}
                </div>
                {walletShort && (
                  <p className="desk-msg">Cüzdanda yeterli kredi yok.</p>
                )}
                {overStock && (
                  <p className="desk-msg">Sepette stoktan fazla gram var.</p>
                )}
                <Button
                  variant="default"
          ...
          <Card asChild className="gap-0 py-0 shadow-none">
            <section className="panel">
              <div className="panel-header">
                <div>
                  <h3 style={{ margin: 0 }}>Sipariş durumu</h3>
                </div>
              </div>
              <div className="panel-body">
