...
/** Family hues for chart bands / heat fills — muted mineral on void. */
...
/** Black or white ink for a solid tile fill (relative luminance). */
export function tileInk(hex: string): "#111111" | "#ffffff" {
...
export function dbCategoryToStaticCategory(dbCategory: string): string {
  ...
  if (norm.includes("alkali metal") && !norm.includes("alkaline"))
    return "alkali";
  if (norm.includes("alkaline earth")) return "alkaline";
  if (norm.includes("transition metal") && !norm.includes("post"))
    return "transition";
...
export function mergeElementData(
  dbElements: ApiElement[],
  staticElements: ElementItem[],
): ElementItem[] {
  return staticElements.map((st) => {
    const db = dbElements.find(
      (item) => item.symbol.toLowerCase() === st.symbol.toLowerCase(),
    );
    ...
    return {
      ...
      phase:
        (
          { Solid: "katı", Liquid: "sıvı", Gas: "gaz" } as Record<
            string,
            string
          >
        )[db?.phase ?? ""] ??
      ...
      electronegativity:
        db?.detail?.electronegativity != null
          ? Number(db.detail.electronegativity)
          : undefined,
      imageUrl: db?.media?.imageUrl,
      sellerName: db?.commerce?.sellerName,
      rating:
        db?.commerce?.rating != null ? Number(db.commerce.rating) : undefined,
