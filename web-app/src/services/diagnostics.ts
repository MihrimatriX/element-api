  ...
  try {
    ...
    return Array.isArray(value)
      ? value
          .filter(
            (v): v is LocalEvent =>
              !!v && typeof v.event === "string" && typeof v.at === "string",
          )
          .slice(-200)
      : [];
  } catch {
...
export function track(event: ProductEvent, item?: string) {
  ...
  if (
    last?.event === event &&
    last.item === safeItem &&
    Date.now() - Date.parse(last.at) < 1000
  )
    return;
  events.push({
    event,
  ...
  try {
    ...
    /* Diagnostics never block discovery. */
  }
}
export function setDiagnostics(enabled: boolean) {
  try {
    localStorage.setItem(DIAGNOSTICS_CONSENT, String(enabled));
