...
const keyFor = (user: string | null) =>
  `elementapi:learning:${user ?? "guest"}:v1`;
const subscribe = (listener: () => void) => {
  window.addEventListener("storage", listener);
  window.addEventListener("element:learning", listener);
  return () => {
    window.removeEventListener("storage", listener);
    window.removeEventListener("element:learning", listener);
  };
};
...
function persist(user: string | null, progress: LearningProgress) {
  const key = keyFor(user),
    value = JSON.stringify(normalizeLearning(progress));
  let saved = true;
  try {
...
export function useLearning() {
    ...
    async function sync() {
      try {
        ...
        if (!controller.signal.aborted)
          setStatus("Hesapla eşitlenemedi. İlerlemen bu cihazda korunuyor.");
        else if (token === readStorage("token"))
          setStatus(
            "Bağlantı zaman aşımına uğradı. İlerlemen bu cihazda korunuyor.",
          );
      } finally {
  ...
  return {
    ...
    status: user
      ? status
      : memory.has(keyFor(null))
        ? "Tarayıcı kaydı kapalı; ilerleme bu oturumda tutuluyor."
        : "Keşiflerin bu tarayıcıda saklanır.",
    save(next: LearningProgress) {
      if (!persist(user, next))
        setStatus("Tarayıcı kaydı kapalı; ilerleme bu oturumda tutuluyor.");
    },
    retry() {
      setAttempt((n) => n + 1);
    },
    ...
    importGuest() {
      persist(user, mergeLearning(progress, parse(raw(null))));
    },
  };
}
