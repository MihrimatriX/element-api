...
export async function sweepExpiredSagas() {
    const res = await pool.query(`SELECT order_id, customer_id, element_symbol, quantity, total_price, current_state
     FROM saga_state
    ...
    for (const row of res.rows) {
        ...
        try {
            ...
            await client.query(`UPDATE saga_state SET current_state = $2, error_message = $3, updated_at = NOW(), deadline_at = NULL
         WHERE order_id = $1`, [ctx.orderId, 'Failed', reason]);
